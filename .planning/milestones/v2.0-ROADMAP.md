...
- [x] **Phase 13: Terminal UX** — Move send box + chips into terminal overlay; remove from card; mobile keyboard-push fix; touch scroll in terminal (completed 2026-03-26)
- [x] **Phase 13.1: Mobile Terminal Polish & Message Log** — Special key buttons (arrows, Tab, Esc, Ctrl+C) for mobile terminal; full-screen send input on mobile; persistent message log with chat-style UI in card drawer (completed 2026-03-27)
- [x] **Phase 14: Telegram Integration** — Merge GSDTelegram into this repo; detect scroll-to-select prompts; send Telegram notification when input is needed (completed 2026-03-28)
- [x] **Phase 16: OOM Prevention** — Add Node.js memory caps, a memory watchdog script, and orphan process cleanup to prevent the Railway claude-code-ssh container from running out of memory when 4+ Claude Code sessions run in parallel (completed 2026-03-28)
...
**Plans**: 3 plans
Plans:
- [x] 12-01-PLAN.md — Backend: detectSessionState() + sessionState in GET /projects + archive/unarchive endpoints
- [x] 12-02-PLAN.md — Frontend: SessionState type + api methods + card border/label + archive UI + archived section
- [x] 12-03-PLAN.md — Frontend: Replace summary stats row with Working/Waiting/Paused/Archived counts
...
**Success Criteria** (what must be TRUE):
  ...
  5. On desktop, the terminal receives keyboard focus immediately when opened so the user can type without clicking first
**Plans**: 2 plans
Plans:
- [x] 13-01-PLAN.md — Move SendBox into TerminalOverlay; remove from ProjectCard; thread initialSendValue
- [x] 13-02-PLAN.md — Mobile keyboard-push fix (visualViewport) and touch scroll (pan-y)

### Phase 13.1: Mobile Terminal Polish & Message Log
**Goal**: Mobile terminal gets special key buttons and full-screen input for usability; a persistent message log shows all GSD/CC communications in a chat-style UI within the card drawer
**Depends on**: Phase 13
**Requirements**: MTERM-01, MTERM-02, MTERM-03, MLOG-01, MLOG-02, MLOG-03
**Success Criteria** (what must be TRUE):
  1. A row of special key buttons (←, →, ↑, ↓, Tab, Esc, Ctrl+C, Enter) appears below the send box in the terminal overlay on mobile devices only
  2. The special key bar replaces the GSD command chips; each button sends the corresponding escape sequence to the tmux session
  3. The send box and special key bar are hidden on desktop (physical keyboard); visible only on touch devices
  4. On mobile, focusing the send box expands it to full-screen width above the on-screen keyboard for easier editing of long messages
  5. A "Messages" tab appears as the first tab in the card drawer/modal, showing a chronological chat-style log of all messages to/from GSD/CC for that project
  6. Messages are persisted server-side (SQLite) with timestamp, project, direction (inbound/outbound), and content; the log survives server restarts
**Plans**: 3 plans
Plans:
- [x] 13.1-01-PLAN.md — Mobile terminal input: special key bar, hide send UI on desktop, full-screen send box on mobile focus
- [x] 13.1-02-PLAN.md — Message log backend: SQLite table, API endpoints, hook into send-keys for outbound logging
- [x] 13.1-03-PLAN.md — Message log frontend: "Messages" tab in card drawer with chat-style UI
...
**Plans**: 2 plans
Plans:
- [x] 14-01-PLAN.md — Telegram module: Bot API wrapper, reply poller, inbound message logging
- [x] 14-02-PLAN.md — State transition detection + scroll-to-select notifications with inline keyboards

### Phase 16: OOM Prevention
**Goal**: The claude-code-ssh Railway container stays within memory limits when running 4+ concurrent Claude Code sessions
**Depends on**: Nothing (infrastructure, independent of feature phases)
**Requirements**: OOM-01, OOM-02, OOM-03
**Success Criteria** (what must be TRUE):
  1. Every node process on the container starts with a capped heap size (≤1GB) via NODE_OPTIONS
  2. A memory watchdog runs in the background, detecting when system memory exceeds 80% and cleaning up orphaned node processes
  3. At 90%+ memory, the watchdog force-kills the largest non-essential node process and logs the action
  4. The watchdog starts automatically on SSH login and survives session disconnects
**Plans**: 1 plan
Plans:
- [x] 16-01-PLAN.md — NODE_OPTIONS cap + memory watchdog script + auto-start via .bashrc + server memory logging
...
| 13. Terminal UX | 2/2 | Complete | 2026-03-26 |
| 13.1 Mobile Terminal Polish & Message Log | 3/3 | Complete | 2026-03-27 |
| 14. Telegram Integration | 2/2 | Complete | 2026-03-28 |
| 16. OOM Prevention | 1/1 | Complete | 2026-03-28 |
