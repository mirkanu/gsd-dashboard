...
- Railway's GSD_DATA_URL proxies task API calls to VPS — **assumption was WRONG**: Railway SQLite is completely empty (no tables at all). GSD_DATA_URL was set from day one so Railway never wrote tasks locally.
- josie, prc, gsdTelegram, zoho-todoist-sync tasks simply don't exist anywhere — they were never created.
...
Migration is a no-op. Accessed Railway container via `railway ssh`, copied `/app/data/dashboard.db` via base64 pipe — file is 4 KB with zero tables (empty SQLite init). `GSD_DATA_URL` was set from the start so Railway never wrote tasks to its local DB. josie, prc, gsdTelegram, zoho-todoist-sync simply have no tasks anywhere.

**No action needed. Thread resolved.**
