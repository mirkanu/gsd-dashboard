...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @/data/home/gsddashboard/.claude/get-shit-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @/data/home/gsddashboard/.claude/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
@/data/home/gsddashboard/.claude/get-shit-done/references/planner-antipatterns.md
...
```bash
INIT=$(node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" graphify status
```
...
```bash
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" graphify query "<phase-goal-keyword>" --budget 2000
```
...
```bash
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
@/data/home/gsddashboard/.claude/get-shit-done/references/thinking-models-planning.md
...
```bash
VALID=$(node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
