...
```bash
INIT=$(node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
@/data/home/gsddashboard/.claude/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@/data/home/gsddashboard/.claude/get-shit-done/references/ios-scaffold.md
...
@/data/home/gsddashboard/.claude/get-shit-done/references/executor-examples.md
</deviation_rules>
...
```bash
AUTO_CHAIN=$(node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**See @/data/home/gsddashboard/.claude/get-shit-done/references/checkpoints.md**
...
```bash
node /data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {concise task description}" --files file1 file2 ...
```
...
**Use template:** @/data/home/gsddashboard/.claude/get-shit-done/templates/summary.md
...
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" state record-metric \
  --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
  --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"

# Add decisions (extract from SUMMARY.md key-decisions)
for decision in "${DECISIONS[@]}"; do
  node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" state add-decision \
    --phase "${PHASE}" --summary "${decision}"
done

# Update session info
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" state record-session \
  --stopped-at "Completed ${PHASE}-${PLAN}-PLAN.md"
```

```bash
# Update ROADMAP.md progress for this phase (plan counts, status)
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"

# Mark completed requirements from PLAN.md frontmatter
# Extract the `requirements` array from the plan's frontmatter, then mark each complete
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
node "/data/home/gsddashboard/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
