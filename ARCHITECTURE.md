...
| `routes/env.js`           | GET /api/env reads `/home/services/.env.production` and returns parsed rows (entry/comment/blank). PUT /api/env atomically overwrites the file via temp-file + rename. Path is hardcoded — no user-supplied path accepted (path traversal prevention). Protected by cookieAuth                                                                                                                                                                                                                                                        |
